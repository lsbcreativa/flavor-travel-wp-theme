...
$page_title = get_theme_mod('flavor_eventos_title', 'Eventos Deportivos');
$page_desc = get_theme_mod('flavor_eventos_desc', 'Vive la emoción de los mejores eventos deportivos del mundo.');
$page_image = get_theme_mod('flavor_eventos_image', '');
if (empty($page_image)) {
    $page_image = 'https://images.unsplash.com/photo-1461896836934- voices-from-athletes?w=1920&q=80';
}

$position = get_theme_mod('flavor_eventos_position', 'center');
$show_badge = get_theme_mod('flavor_eventos_show_badge', true);
$show_cta = get_theme_mod('flavor_eventos_show_cta', true);
$cta_text = get_theme_mod('flavor_eventos_cta_text', 'Consultar por WhatsApp');
$cta_url = get_theme_mod('flavor_eventos_cta_url', '');
$show_scroll = get_theme_mod('flavor_eventos_show_scroll', true);

if (empty($cta_url)) {
    $cta_url = 'https://wa.me/' . get_theme_mod('flavor_whatsapp', '00123456789') . '?text=' . urlencode('Hola, me interesa información sobre eventos deportivos');
}
...
.eventos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px; }
.evento-card {
    background: white;
    border-radius: 16px;
    overflow: hidden;
...
<section style="position: relative; min-height: 100vh; min-height: 100dvh; display: flex; align-items: <?php echo $align_items; ?>; overflow: hidden;">
    ...
    <div class="hero-content" style="position: relative; z-index: 10; max-width: 1280px; margin: 0 auto; padding: <?php echo $padding_top; ?> 20px 100px; width: 100%;">
        ...
        <span style="display: inline-flex; align-items: center; gap: 8px; background: #dc2626; color: white; padding: 8px 20px; border-radius: 50px; font-size: 0.85rem; margin-bottom: 20px;">
            🏆 <?php echo $total; ?> eventos disponibles
        </span>
        <?php endif; ?>
    ...
    <div class="scroll-indicator" style="position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); text-align: center; color: white; cursor: pointer;" onclick="document.querySelector('.content-section').scrollIntoView({behavior: 'smooth'})">
        <div style="font-size: 0.85rem; margin-bottom: 8px; opacity: 0.8;">Ver eventos</div>
...
<section class="content-section" style="background: #f8fafc; padding: 80px 0;">
    <div style="max-width: 1280px; margin: 0 auto; padding: 0 20px;">
        ...
        <div class="eventos-grid">
            ...
            <article class="evento-card">
                <a href="<?php the_permalink(); ?>" class="img-wrap">
                    ...
                    <span class="badge-deportivo">🏆 Evento</span>
                    <?php if ($duracion): ?>
                ...
                <div style="padding: 24px;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 16px; border-top: 1px solid #e2e8f0;">
                        ...
                        <a href="<?php the_permalink(); ?>" style="background: #dc2626; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 0.9rem;">Ver detalles</a>
                    </div>
                </div>
            </article>
            <?php endwhile; ?>
        ...
        <div style="text-align: center; padding: 80px 20px; background: white; border-radius: 16px;">
            ...
            <p style="color: #64748b; font-size: 1.1rem; margin-bottom: 24px;">Aún no hay eventos deportivos publicados.</p>
            <a href="<?php echo home_url('/contacto/'); ?>" style="display: inline-block; background: #dc2626; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600;">Contáctanos para más información</a>
        </div>
        <?php endif; ?>
    </div>
</section>
