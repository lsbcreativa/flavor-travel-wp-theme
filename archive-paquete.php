...
$page_title = get_theme_mod('flavor_tours_title', 'Tours');
$page_desc = get_theme_mod('flavor_tours_desc', 'Descubre experiencias únicas en cada rincón del mundo.');
$page_image = get_theme_mod('flavor_tours_image', '');
if (empty($page_image)) {
    $page_image = 'https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1920&q=80';
}

$position = get_theme_mod('flavor_tours_position', 'center');
$show_badge = get_theme_mod('flavor_tours_show_badge', true);
$hide_empty_badge = get_theme_mod('flavor_tours_hide_empty_badge', true);
$show_cta = get_theme_mod('flavor_tours_show_cta', true);
$cta_text = get_theme_mod('flavor_tours_cta_text', 'Consultar por WhatsApp');
$cta_url = get_theme_mod('flavor_tours_cta_url', '');
$show_scroll = get_theme_mod('flavor_tours_show_scroll', true);

if (empty($cta_url)) {
    $cta_url = 'https://wa.me/' . get_theme_mod('flavor_whatsapp', '00123456789') . '?text=' . urlencode('Hola, me interesa información sobre tours');
}
...
<section style="position: relative; min-height: 100vh; min-height: 100dvh; display: flex; align-items: <?php echo $align_items; ?>; overflow: hidden;">
    ...
    <div class="scroll-indicator" style="position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); text-align: center; color: white; cursor: pointer;" onclick="document.querySelector('.content-section').scrollIntoView({behavior: 'smooth'})">
        <div style="font-size: 0.85rem; margin-bottom: 8px; opacity: 0.8;">Ver tours</div>
...
<section class="content-section" style="background: #f8fafc; padding: 80px 0;">
    <div style="max-width: 1280px; margin: 0 auto; padding: 0 20px;">
        ...
        <div style="text-align: center; padding: 80px 20px; background: white; border-radius: 16px;">
            ...
            <p style="color: #64748b; font-size: 1.1rem; margin-bottom: 24px;">Aún no hay tours publicados.</p>
            <a href="<?php echo home_url('/contacto/'); ?>" style="display: inline-block; background: #2563eb; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600;">Contáctanos para crear tu tour</a>
        </div>
        <?php endif; ?>
    </div>
</section>
...
<?php 
$seo_text = get_theme_mod('flavor_tours_seo_text', '');
$seo_image = get_theme_mod('flavor_tours_seo_image', '');
$seo_layout = get_theme_mod('flavor_tours_seo_layout', 'text-only');
